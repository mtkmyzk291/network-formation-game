...
public class LinkOptimizer_8_Simple {
	...
	public static void main(String[] args) {
		...
		LinkOptimizer_8_Simple lo = new LinkOptimizer_8_Simple();
		List<Agent> agentList = new ArrayList<>();

		int allLink = 4;

		for (int i = 0; i < 200; i++) { // iの上限でエージェント数変更
		...
		for (int p = 0; p < 2; p++) {
			...
			expectationAry = calcMyExpectNoWrite(agentList, countedDistanceAry);
			// ネットワーク全体の得点算出
			...
			// ここから期待値[ここは要る]
			try {
				Writer writer = new FileWriter("140217_nwExpectation.txt", true);
			...
			// try {
			// Writer writer = new FileWriter("140217_nwExpectation_random.txt",
			// true);
			// BufferedWriter bw = new BufferedWriter(writer);
			//
	...
	public LinkOptimizer_8_Simple(Random rand) {
		this.rand = rand;
	}

	public LinkOptimizer_8_Simple() {
	...
	public static double[] calcMyExpectFirst(List<Agent> agentList, int[][] countedDistanceAry) {
		...
		for (Agent agent : agentList) {
			...
			try {
				Writer writer = new FileWriter("140217_Expectation.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
			...
			try {
				Writer writer = new FileWriter("140217_Expectation_random.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
	...
	public static void chooseNewLink(int agentNum, List<Agent> agentList, double[] expectationAry, int maxLinkNum) {
		...
		for (int p = 0; p < maxLinkNum; p++) {
			...
			lastExpectation = calcMyExpectNoWrite(copyList, preCountedDistanceAry);
			...
			for (int i = 0; i < copyList.size(); i++) {
				...
				if (nextExpect > linkedCost) {// コストを上回る期待値をそのノードから得られれば新規リンク作成
					...
					// ここから新規リンク[ここは要る]
					try {
						Writer writer = new FileWriter("140217_newLink.txt", true);
						BufferedWriter bw = new BufferedWriter(writer);

						bw.write(receiver.getId() + "\t" + candidate.getId() + "\t"
								+ receiver.getLinkedNodeSet().size() + "\n");
			...
			// if (p == maxLinkNum - 1) {
			newExpectationAry = calcMyExpectNoWrite(copyList, countedDistanceAry);
			// } else {
	...
	public static void chooseNewLinkRandom(int agentNum, List<Agent> agentList, int maxLinkNum) {
		...
		for (int p = 0; p < maxLinkNum; p++) {
			...
			// ここから期待値[ここは要る]
			try {
				Writer writer = new FileWriter("140217_nwExpectation_random.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
