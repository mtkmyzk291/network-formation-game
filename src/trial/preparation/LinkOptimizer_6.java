...
public class LinkOptimizer_6 {
	...
	public static void main(String[] args) {
		...
		LinkOptimizer_6 lo = new LinkOptimizer_6();
...
//			generator.createTwoWaySmallWorldConstLinkNum(agentList, allLink, 0.1, 0.05, 0.05); // 全リンク数、張り替え率、リンクのランダムさ、リンクの単方向性
			...
			expectationAry = calculateMyExpect(agentList, countedDistanceAry);
			// ネットワーク全体の得点算出
			...
			chooseNewLink(agentNum, agentList, expectationAry);
		}
	}

	private final Random rand;

	public LinkOptimizer_6(Random rand) {
		this.rand = rand;
	}

	public LinkOptimizer_6() {
	...
	public static double[] calculateMyExpect(List<Agent> agentList, int[][] countedDistanceAry) {
		int agentNum = agentList.size();
	...
	public static void chooseNewLink(int agentNum, List<Agent> agentList, double[] expectationAry) {
		...
		lastExpectation = calculateMyExpect(copyList, preCountedDistanceAry);
		...
		for (int p = 0; p < maxLinkNum; p++) {
			...
			// 全エージェントに1つずつリンクを追加
			for (int i = 0; i < copyList.size(); i++) {
				double maxExpectation = 2 * expectationAry[i];
				if (nextLinkList.get(i) > copyList.size()) {
					continue;
				} else if (maxExpectation < (lastExpectation[i] + nextPlusList.get(i))) {
					continue;
				} else {
			...
			newExpectationAry = calculateMyExpect(agentList, countedDistanceAry);
