...
import trial.preparation.Agent;
...
public class ThreeSteps {
	...
	public static void main(String[] args) {
		...
		ThreeSteps ts = new ThreeSteps();
		List<Agent> agentList = new ArrayList<>();

		int defaultLink = 6;
		int counter = 20; // 1NWあたりの繰り返し回数

		int agentNum=200; //エージェント数
		int totalStep=5; //全体の繰り返し回数
		
		for (int i = 0; i < agentNum; i++) { // iの上限でエージェント数変更
			agentList.add(new Agent());
		}
		for (int p = 0; p < totalStep; p++) { //ゲームの繰り返し回数p
			System.out.println(p + "回目");
			for (Agent agent : agentList) {
				agent.setInfoValue(ts.setInfoValue(agent.getInfoValue())); // 各エージェントの持つ情報の価値を設定
				// System.out.println(agent.infoValue);
			}
			
			twGenerator.createTwoWaySmallWorld(agentList, defaultLink, 0.1);

			for (int j = 0; j < agentNum; j++) {
				System.out.println("linkpair_start");
				try {
					Writer writer = new FileWriter("140016_linkpair.txt", true);
				...
				for (Node linked : agentList.get(j).getLinkedNodeSet()) {
					...
					try {
						Writer writer = new FileWriter("140416_linkpair.txt", true);
						BufferedWriter bw = new BufferedWriter(writer);
			...
			try {
				Writer writer = new FileWriter("140416_newLink.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
			...
			// ここから期待値[ここは要る]
			try {
				Writer writer = new FileWriter("140416_nwExpectation.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
...
//			}
	...
	public static void chooseNewLinkRandom(int agentNum, List<Agent> agentList, int maxLinkNum) {
		...
		for (int p = 0; p < maxLinkNum; p++) {
			...
			// ここから期待値[ここは要る]
			try {
				Writer writer = new FileWriter("140416_nwExpectation_random.txt", true);
				BufferedWriter bw = new BufferedWriter(writer);
		...
		try {
			Writer writer = new FileWriter("140416_Expectation_random.txt", true);
			BufferedWriter bw = new BufferedWriter(writer);
